# OmniLauncher-MC End-to-End Tests
...
|---|---|---|---|
| **Safety-net (兜底)** | [`specs/ci/`](specs/ci) | Isolated, deterministic — a throwaway temp profile per test, no live-network installs, Java launch stubbed. Launches the flat `omnilauncher-electron-app/dist/`. | Automatically in CI/CD on every push / PR |
...
pnpm build:renderer && pnpm --prefix=omnilauncher-electron-app compile
pnpm test:e2e:ci         # safety-net group (fast, deterministic — the CI gate)
pnpm test:e2e:release    # packaged-boot group (needs `omnilauncher-electron-app build` first)
...
| Concern | Mechanism |
|---|---|
| Launcher root | Per-test temp dir injected via `OMNI_E2E_APP_DATA` |
| Auto-updater | Disabled when `OMNI_E2E` is set (see `pluginAutoUpdate.ts`) |
| Java spawn | Stubbed when `OMNI_E2E_NO_LAUNCH` is set (see `LaunchService.ts`) |
...
| Main-process network | Planned: `OMNI_E2E_MOCKS_FILE` JSON intercept |
...
3. Add `data-testid` attributes (not text/class selectors) for any new
   anchors in `omnilauncher-ui/` and surface them in
...
```bash
OMNI_E2E_LOCALES=en,zh-CN pnpm test:e2e:showcase
```
