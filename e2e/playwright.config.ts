...
// Adding more locales is a one-line change: read process.env.OMNI_E2E_LOCALES.
const locales = (process.env.OMNI_E2E_LOCALES ?? 'en').split(',').map((s) => s.trim())
...
export default defineConfig({
  ...
  use: {
    ...
    // Default keeps videos only for failures. Set OMNI_E2E_VIDEO=all to
    // record the full run (used for local showcase capture / promo footage).
    video: process.env.OMNI_E2E_VIDEO === 'all' ? 'on' : 'retain-on-failure',
  },
  projects: locales.map((locale) => ({
    ...
    metadata: {
      locale,
      electronAppPath: resolve(__dirname, '../omnilauncher-electron-app/dist/index.js'),
    },
  })),
})
