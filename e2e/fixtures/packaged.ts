 ...
 * `omnilauncher-electron-app/dist/index.js` directory with the workspace Electron
 * binary — this fixture launches the **real artifact** produced by
 * `electron-builder --dir` (i.e. `pnpm --prefix omnilauncher-electron-app build`),
...
const OUTPUT_ROOT = resolve(REPO_ROOT, 'omnilauncher-electron-app/build/output')
...
function findPackagedBinary(): string {
  const productName = 'OmniLauncher-MC'
  ...
  return join(OUTPUT_ROOT, 'linux-unpacked', 'omnilauncher-mc')
}
...
export const test = base.extend<{ packaged: PackagedFixture }>({
  packaged: async ({}, use, testInfo) => {
    ...
    if (!existsSync(bin)) {
      throw new Error(
        `Packaged binary not found at ${bin}. Run \`pnpm --prefix omnilauncher-electron-app build\` ` +
