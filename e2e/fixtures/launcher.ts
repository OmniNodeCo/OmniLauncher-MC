 ...
 *  - Per-test temp directory for both Electron `userData` and the OmniLauncher-MC game
 *    data root. No shared state between tests.
 *  - Auto-updater disabled via OMNI_E2E env var.
 *  - Real `java` spawn short-circuited via OMNI_E2E_NO_LAUNCH.
...
const ELECTRON_ENTRY = resolve(REPO_ROOT, 'omnilauncher-electron-app/dist/index.js')
...
const ELECTRON_BIN = process.platform === 'win32'
  ? resolve(REPO_ROOT, 'omnilauncher-electron-app/node_modules/electron/dist/electron.exe')
  : process.platform === 'darwin'
    ? resolve(REPO_ROOT, 'omnilauncher-electron-app/node_modules/electron/dist/Electron.app/Contents/MacOS/Electron')
    : resolve(REPO_ROOT, 'omnilauncher-electron-app/node_modules/electron/dist/electron')
...
export const test = base.extend<{
  ...
  launcher: async ({ launcherOptions }, use, testInfo) => {
    ...
    // Layout matches what omnilauncher-runtime/app/LauncherApp.ts expects:
    ...
    if (!launcherOptions.bootstrap) {
      const omniDir = join(appDataPath, 'OmniLauncher-MC')
      await mkdir(omniDir, { recursive: true })
      await writeFile(join(omniDir, 'root'), gameDataPath)
    }
