 ...
 *      hook (installed in omnilauncher-electron-app/main/index.ts under OMNI_E2E).
 *
    ...
    try {
      ...
      const result = await packaged.app.evaluate(async (_electron, target) => {
        ...
        if (typeof probe !== 'function') {
          throw new Error('__xmclE2EProbeHttp hook missing — is OMNI_E2E set for the packaged app?')
        }
