...
// `@/` is the renderer's path alias (omnilauncher-ui/src). Map it so unit
// tests can import composables/utils that use it. The regex form only matches
// `@/...` and leaves scoped packages like `@xmcl/...` untouched.
const keystoneSrc = join(__dirname, 'omnilauncher-ui/src').replace(/\\/g, '/')
const runtimeSrc = join(__dirname, 'omnilauncher-runtime').replace(/\\/g, '/')
