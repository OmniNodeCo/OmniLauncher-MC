...
case "$TYPE" in
    ...
    exe)
        ...
        APP_SRC=$(cygpath -w "$APP_DIR" 2>/dev/null || echo "$APP_DIR")
        SETUP_ICON=$(cygpath -w build/icons/OmniLauncher.ico 2>/dev/null || echo build/icons/OmniLauncher.ico)
        OUT_DIR=$(cygpath -w build/dist 2>/dev/null || echo build/dist)
...
[Files]
Source: "$APP_SRC\*"; DestDir: "{app}"; Flags: recursesubdirs createallsubdirs ignoreversion
