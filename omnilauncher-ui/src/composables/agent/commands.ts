...
export function assertAgentCommandSyntax(argv: string[]) {
  const unsupported = argv.find(token =>
    ...
    token.includes('$(') ||
    token.includes('`'))
  if (unsupported) throw new Error(`Unsupported shell syntax: ${unsupported}. The vfs_shell tool accepts one OmniLauncher-MC command only.`)
}
  ...
  return [
    {
      ...
      description: 'Search and read built-in OmniLauncher-MC workflow documentation.',
      details: [
        'document list',
