...
class OmniLauncher(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.geometry(
            f"{self.settings_mgr.get('window_width')}x{self.settings_mgr.get('window_height')}"
        )
    ...
    def _build_home_page(self):
        ...
        if active:
            ...
            version_text = "Version: %s  |  Loader: %s" % (
                resolved,
                active.loader.title(),
            )
        else:
        ...
        for label, value in stats:
            card = ctk.CTkFrame(
                stats_frame, fg_color=COLORS["bg_card"], corner_radius=12
            )
    ...
    def _on_profile_select(self, name: str):
        ...
        if profile:
            ...
            self.version_info_label.configure(
                text="Version: %s  |  Loader: %s"
                % (resolved, profile.loader.title())
            )
    ...
    def _build_profiles_page(self):
        ...
        for profile in self.profile_mgr.profiles:
            ...
            ctk.CTkLabel(
                info,
                text="%s  |  %s" % (resolved, profile.loader.title()),
                font=ctk.CTkFont(size=12),
                text_color=COLORS["text_dim"],
            ).pack(anchor="w")
    ...
    def _build_versions_page(self):
        ...
        for version in versions[:40]:
            ...
            card = ctk.CTkFrame(
                page, fg_color=COLORS["bg_card"], corner_radius=12
            )
    ...
    def _build_settings_page(self):
        ...
        def save_username():
            ...
            if self.auth_mgr.login_offline(name):
                ...
                self.status_label_settings.configure(
                    text="Invalid username (3-16 alphanumeric chars)",
        ...
        ctk.CTkLabel(
            java_card,
            text="Java Executable Path (leave empty for auto-detect)",
            font=ctk.CTkFont(size=12),
            text_color=COLORS["text_secondary"],
        ).pack(anchor="w")
        ...
        ram_label = ctk.CTkLabel(
            java_card,
            text="Maximum RAM: %d MB" % ram_value,
            font=ctk.CTkFont(size=12),
            text_color=COLORS["text_secondary"],
        )
        ...
        show_snap_var = ctk.BooleanVar(
            value=self.settings_mgr.get("show_snapshots")
        )
        ctk.CTkCheckBox(
            game_card,
            ...
            command=lambda: self.settings_mgr.set(
                "show_snapshots", show_snap_var.get()
            ),
        ).pack(anchor="w", pady=4)

        close_var = ctk.BooleanVar(
            value=self.settings_mgr.get("close_on_launch")
        )
        ctk.CTkCheckBox(
            game_card,
