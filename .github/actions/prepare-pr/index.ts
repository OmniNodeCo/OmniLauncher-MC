...
const appPackageDirs = ['omnilauncher-ui', 'omnilauncher-runtime', 'omnilauncher-runtime-api']
...
function writeVersions(packages: PackageInfo[], rootVersion: string) {
  ...
  const electronFile = 'omnilauncher-electron-app/package.json'
  ...
  for (const pkg of packages.sort((a, b) => Number(!!a.passive) - Number(!!b.passive))) {
    ...
    for (const reason of pkg.reasons) {
      if (typeof reason === 'string') {
        ...
        body += `  - ${reason.header} ([${reason.hash}](https://github.com/OmniNodeCo/OmniLauncher-MC/commit/${reason.hash}))\n`
      }
    }
  }
  return body
...
function getAppChangelog(version: string) {
  ...
  for (const section of sections) {
    ...
    for (const commit of matched) {
      ...
      body += `- ${scope}${commit.subject || commit.header} ([${commit.hash}](https://github.com/OmniNodeCo/OmniLauncher-MC/commit/${commit.hash}))\n`
    }
  }
  return body
}
