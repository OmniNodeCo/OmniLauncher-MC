...
async function buildMain(
  ...
  await copy(path.join(__dirname, '../omnilauncher-ui/dist'), path.join(__dirname, './dist/renderer'))
  ...
  const config: Configuration = {
    ...
    async artifactBuildCompleted(context) {
      ...
      if (context.target && context.target.name === 'appx') {
        await buildAppInstaller(version, path.join(__dirname, './build/output/OmniLauncher-MC.appinstaller'), electronBuilderConfig.appx!.publisher!)
      }
    },
  }
