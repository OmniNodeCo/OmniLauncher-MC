...
export default class ElectronLauncherApp extends LauncherApp {
  ...
  constructor() {
    // E2E hook: when OMNI_E2E_APP_DATA is set, redirect Electron's appData
    // (and therefore both `app.getPath('appData')` calls below + the one in
    // omnilauncher-runtime/app/LauncherApp.ts) to a per-test temporary directory.
    // This isolates every Playwright spec on a clean launcher root.
    if (process.env.OMNI_E2E_APP_DATA) {
      try {
        app.setPath('appData', process.env.OMNI_E2E_APP_DATA)
        app.setPath('userData', join(process.env.OMNI_E2E_APP_DATA, LAUNCHER_NAME))
      } catch {
