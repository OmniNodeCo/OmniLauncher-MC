...
const displayType = koffi.pointer(koffi.opaque('OmniLauncherDisplay'))
const clientMessage = koffi.struct('OmniLauncherXClientMessageEvent', {
  type: 'int',
  serial: 'ulong',
  sendEvent: 'int',
  display: displayType,
...
export async function moveX11Window(pid: number, bounds: MonitorBounds) {
  ...
  const openDisplay = x11.func('OmniLauncherDisplay *XOpenDisplay(const char *name)')
  const closeDisplay = x11.func('int XCloseDisplay(OmniLauncherDisplay *display)')
  const defaultScreen = x11.func('int XDefaultScreen(OmniLauncherDisplay *display)')
  const rootWindow = x11.func('unsigned long XRootWindow(OmniLauncherDisplay *display, int screen)')
  const internAtom = x11.func('unsigned long XInternAtom(OmniLauncherDisplay *display, const char *name, bool onlyIfExists)')
  const getWindowProperty = x11.func('int XGetWindowProperty(OmniLauncherDisplay *display, unsigned long window, unsigned long property, long offset, long length, bool deleteProperty, unsigned long requestedType, _Out_ unsigned long *actualType, _Out_ int *actualFormat, _Out_ unsigned long *itemCount, _Out_ unsigned long *bytesAfter, _Out_ void **data)')
  const free = x11.func('int XFree(void *data)')
  const moveResizeWindow = x11.func('int XMoveResizeWindow(OmniLauncherDisplay *display, unsigned long window, int x, int y, uint32_t width, uint32_t height)')
  const flush = x11.func('int XFlush(OmniLauncherDisplay *display)')
  const sendEvent = x11.func('int XSendEvent(OmniLauncherDisplay *display, unsigned long window, bool propagate, long eventMask, const OmniLauncherXClientMessageEvent *event)')
