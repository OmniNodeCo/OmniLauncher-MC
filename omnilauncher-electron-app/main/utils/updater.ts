...
async function downloadAsarUpdate(
  ...
  const github = `https://github.com/OmniNodeCo/OmniLauncher-MC/releases/download/v${version}/${file}`
...
async function hintUserDownload(): Promise<void> {
  shell.openExternal('https://github.com/OmniNodeCo/OmniLauncher-MC')
}
...
async function downloadAppInstaller(
  ...
  const destination = join(app.getPath('downloads'), 'OmniLauncher-MC.appinstaller')
      ...
      Object.assign(executor, {
        ...
        createRequest: (options: any, callback: any) => {
          if (gfw.inside) {
            ...
            options.pathname = `/Soft_Mirrors/github-release/Voxelum/OmniLauncher-MC/LatestRelease/${basename(options.pathname)}`
