...
import { createXmclDpopProof, generateXmclDpopKey, type XmclDpopKey } from './OmniLauncherAccountDpop'

export const M1_LOCAL_CONTRACT_VERSION = 'm1-local-proposal-2026-07-22+shared-v1'
export const OMNI_SHARED_CONTRACT_VERSION = 'shared/v1'
...
export class OmniLauncherAccountApiError extends Error {
  constructor(
    ...
    super(code)
    this.name = 'OmniLauncherAccountApiError'
  }
}
...
export class OmniLauncherAccountApi {
  ...
  async exchangeBrowser(
    ...
    if (linking) {
      ...
      if (!snapshot.account) throw new TypeError('Missing OmniLauncher-MC account after identity link')
      return {
  ...
  async getTogetherOverview(credential: XmclSessionCredential): Promise<XmclTogetherOverview> {
    const [offer, trial, subscription, allowances, balance] = await Promise.all([
      ...
      this.request<unknown>('/v1/xmcl-plus/trial', { method: 'GET' }, credential, undefined, this.togetherBaseUrl).catch((error) => {
        if (error instanceof OmniLauncherAccountApiError && error.status === 404) {
          return {
            status: 'unavailable',
  ...
  private async request<T = unknown>(
    ...
    if (!response.ok) {
      ...
      throw new OmniLauncherAccountApiError(
        response.status,
...
function parseAccount(value: unknown): XmclAccount {
  if (
    ...
    throw new TypeError('Invalid OmniLauncher-MC account response')
  }
  return {
