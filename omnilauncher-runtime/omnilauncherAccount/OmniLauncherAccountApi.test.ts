...
import { OmniLauncherAccountApi, OmniLauncherAccountApiError } from './OmniLauncherAccountApi'
import { generateXmclDpopKey } from './OmniLauncherAccountDpop'
import { M1_LOCAL_AUTH_FIXTURE, M1_SHARED_V1_POLICY_FIXTURE } from './fixtures'

describe('OmniLauncherAccountApi', () => {
  it('uses the injected OmniLauncher-MC API origin', async () => {
    const fetch = vi.fn(async (_input: string | URL, _init?: RequestInit) =>
      Response.json(M1_LOCAL_AUTH_FIXTURE),
    )
    const api = new OmniLauncherAccountApi(fetch, 'https://edge.example.test/')
  ...
  it('keeps provider and OmniLauncher-MC credentials in main-process requests', async () => {
    const fetch = vi.fn(async (_input: string | URL, _init?: RequestInit) =>
      Response.json(M1_LOCAL_AUTH_FIXTURE),
    )
    const api = new OmniLauncherAccountApi(fetch)
  ...
  it('binds the browser session to the P-256 public JWK submitted at exchange', async () => {
    ...
    const api = new OmniLauncherAccountApi(
      fetch,
  ...
  it('retries the DPoP key provider after a transient failure', async () => {
    ...
    const api = new OmniLauncherAccountApi(fetch, 'https://edge.example.test/', keyProvider)
    const request = {
  ...
  it('rejects M6 accounting fields on the D1/D4 policy endpoint', async () => {
    const api = new OmniLauncherAccountApi(async () =>
      Response.json({
  ...
  it('reports identity conflicts without exposing response details', async () => {
    ...
    expect(error).toBeInstanceOf(OmniLauncherAccountApiError)
  ...
  it('keeps billing available when the deployment has not published the trial route', async () => {
    const fetch = vi.fn(async (input: string | URL) => {
      ...
      if (path === '/v1/xmcl-plus/offer') {
        return Response.json({
          offerId: 'xmcl-plus',
          displayName: 'OmniLauncher-MC Together Home',
