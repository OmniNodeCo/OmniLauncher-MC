...
import { OmniLauncherAccountApiError, XmclAccountSessionResponseError } from './OmniLauncherAccountApi'
import { generateXmclDpopKey, serializeXmclDpopKey } from './OmniLauncherAccountDpop'
...
const { OmniLauncherAccountService, kOmniSessionAuthorization } = await import('./OmniLauncherAccountService')
...
function createXmclService(stubBootstrapCredential = true) {
  ...
  const service = new OmniLauncherAccountService(app as any, {} as any, store as any)
...
it('registers the server-approved browser OAuth callback path', () => {
  ...
  handler({
    request: {
      url: new URL('omni://launcher/commercial-auth?state=unknown'),
    },
    response,
  })
...
describe('OmniLauncherAccountService authentication', () => {
  ...
  it('authorizes the OmniLauncher-MC Microsoft identity interactively without a game account login', async () => {
  ...
  it('persists the rotated credential when the subsequent account snapshot fails', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'OmniLauncher-MC session refreshed; account snapshot refresh will retry later.',
    )
  })
})

describe('OmniLauncherAccountService automatic session refresh', () => {
  it('single-flights refresh before expiry and returns the rotated access token', async () => {
    ...
    const first = (service as any)[kOmniSessionAuthorization]()
    const second = (service as any)[kOmniSessionAuthorization]()
  ...
  it('does not refresh a token outside the expiry window', async () => {
    ...
    await expect((service as any)[kOmniSessionAuthorization]()).resolves.toEqual({
      accessToken: 'current-access-token',
      accountId: 'account-1',
    })
  ...
  it('clears an expired session when manual refresh is terminal', async () => {
    ...
    ;(service as any).api.refreshSession = vi
      .fn()
      .mockRejectedValue(new OmniLauncherAccountApiError(401, 'refresh_token_expired'))
  ...
  it('clears terminal session state even when secure storage cleanup fails', async () => {
    ...
    ;(service as any).api.refreshSession = vi
      .fn()
      .mockRejectedValue(new OmniLauncherAccountApiError(401, 'session_revoked'))
    app.secretStorage.put.mockRejectedValueOnce(new Error('secure storage unavailable'))

    await expect((service as any)[kOmniSessionAuthorization]()).resolves.toBeUndefined()
    expect((service as any).credential).toBeUndefined()
    expect((service as any).state.account).toBeUndefined()
    expect(logger.warn).toHaveBeenCalledWith(
      'OmniLauncher-MC session cleared in memory, but secure storage cleanup failed.',
    )
  })
})

describe('OmniLauncherAccountService provider bootstrap queue', () => {
  ...
  it('keeps a successful session when the post-auth identity refresh fails', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'OmniLauncher-MC authentication succeeded; account identities will refresh later.',
    )
  })
  ...
  it('ignores a delayed identity conflict after the session changes', async () => {
    ...
    await (service as any).clearSession()
    rejectExchange(new OmniLauncherAccountApiError(409, 'identity_conflict', undefined, 'merge-1'))
  ...
  it('does not send an expired OmniLauncher-MC credential during provider re-bootstrap', async () => {
    const { service } = createXmclService(false)
    const expired = {
...
describe('OmniLauncherAccountService DPoP device key lifecycle', () => {
