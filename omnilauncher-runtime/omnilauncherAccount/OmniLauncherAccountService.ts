import {
  OmniLauncherAccountServiceKey,
  XmclAccountState,
  type XmclAccountSnapshot,
  type XmclOAuthProvider,
  type OmniLauncherAccountService as IOmniLauncherAccountService,
  type SharedState,
} from '@xmcl/runtime-api'
import { createHash, randomBytes } from 'crypto'
import { Inject, LauncherApp, LauncherAppKey } from '~/app'
import { resolveOmniLauncherApiEndpoints } from '~/app/omnilauncherApiBaseUrl'
...
import {
  OmniLauncherAccountApi,
  OmniLauncherAccountApiError,
  XmclAccountSessionResponseError,
  type XmclAuthResult,
  type XmclSessionCredential,
  toSessionSummary,
} from './OmniLauncherAccountApi'
import {
  createXmclDpopProof,
  generateXmclDpopKey,
  restoreXmclDpopKey,
 ...
 */
export const kOmniSessionAuthorization = Symbol('xmcl-session-authorization')

@ExposeServiceKey(OmniLauncherAccountServiceKey)
export class OmniLauncherAccountService
  extends StatefulService<XmclAccountState>
  implements IOmniLauncherAccountService
{
  private readonly api: OmniLauncherAccountApi
  ...
  constructor(
    ...
    super(
      app,
      () => store.registerStatic(new XmclAccountState(), OmniLauncherAccountServiceKey),
      () => this.restore(),
    )
    this.api = new OmniLauncherAccountApi(
      (input, init) => app.fetch(input, init),
      async () => {
        const flights = await app.registry.get(kFlights)
        return resolveOmniLauncherApiEndpoints(
          flights.omnilauncherApiBaseUrl,
          () => app.getLogger('ApiBaseUrl').warn('Ignoring invalid xmclApiUrl flight; using default OmniLauncher-MC API origins.'),
        ).common
      },
      () => this.getDpopKey(),
      async () => {
        ...
        return resolveOmniLauncherApiEndpoints(
          ...
          () => app.getLogger('BillingApiBaseUrl').warn('Ignoring invalid xmclBillingApiBaseUrl flight; using the default OmniLauncher-MC API origin.'),
        ).common
      },
    )

    app.protocol.registerHandler('omnilauncher', ({ request, response }) => {
      if (
  ...
  async [kOmniSessionAuthorization](
  ...
  async refreshSession(): Promise<void> {
    ...
    try {
      ...
      this.warn('OmniLauncher-MC session refreshed; account snapshot refresh will retry later.')
    }
  }
  ...
  private async bootstrapCredential(
    ...
    return this.enqueueProviderBootstrap(async () => {
      ...
      try {
        ...
        if (error instanceof OmniLauncherAccountApiError && error.code === 'identity_conflict') {
  ...
  private async applyAuthResult(result: XmclAuthResult, expectedGeneration = this.sessionGeneration) {
    ...
    if (result.identities === undefined) {
      ...
      try {
        ...
        this.warn('OmniLauncher-MC authentication succeeded; account identities will refresh later.')
      }
    }
  }
  ...
  private async getValidCredential(): Promise<XmclSessionCredential | undefined> {
    ...
    try {
      ...
      if (fallback && Number.isFinite(fallbackExpiresAt) && fallbackExpiresAt > Date.now()) {
        this.warn('OmniLauncher-MC session refresh failed; the current access token will be used until expiry.')
        return fallback
      }
      this.warn('OmniLauncher-MC session refresh failed and the current access token is expired.')
      return undefined
    }
  }
  ...
  private async loadDpopKey(stored: StoredXmclSession): Promise<XmclDpopKey | undefined> {
    ...
    if (embeddedKey) {
      try {
        ...
        this.warn('OmniLauncher-MC DPoP key migration could not create the dedicated copy.')
      }
    }
    return embeddedKey
  }
  ...
  private removeEmbeddedDpopKey(stored: StoredXmclSession) {
    ...
    return this.app.secretStorage
      ...
      .catch(() => {
        this.warn('OmniLauncher-MC DPoP key migration retained the legacy embedded copy.')
      })
  }
  ...
  private clearSession(expectedSessionId?: string) {
    return this.enqueueSessionMutation(async () => {
      ...
      try {
        ...
        this.warn('OmniLauncher-MC session cleared in memory, but secure storage cleanup failed.')
      }
      return true
    })
  }
  ...
  private recordError(error: unknown) {
    const code =
      error instanceof OmniLauncherAccountApiError
        ? error.code
    ...
    this.state.operationError({
      code,
      message: 'OmniLauncher-MC account request failed',
    })
  }
}
