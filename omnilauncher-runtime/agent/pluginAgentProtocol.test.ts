...
describe('agent protocol', () => {
  test('routes custom mode to its endpoint with its stored key', async () => {
    ...
    const ctx = context(BUILTIN_AGENT_ENDPOINT, {
      ...
      'X-OmniLauncher-MC-Account-ID': 'renderer-account',
    })
  ...
  test('authenticates builtin mode with the OmniLauncher-MC session and account', async () => {
    const handler = createAgentProtocolHandler(
      async () => ({ mode: 'builtin' }),
      async () => ({ accessToken: 'xmcl-token', accountId: 'account-id' }),
    )
  ...
  test('rejects builtin mode without an OmniLauncher-MC session', async () => {
    const handler = createAgentProtocolHandler(
      async () => ({ mode: 'builtin' }),
      async () => undefined,
    )
  ...
  test('rejects unconfigured mode without resolving OmniLauncher-MC authorization', async () => {
    const resolveXmclAuthorization = vi.fn(async () => ({
      accessToken: 'xmcl-token',
      accountId: 'account-id',
    }))
