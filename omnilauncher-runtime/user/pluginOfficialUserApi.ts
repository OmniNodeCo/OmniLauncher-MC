...
const XBOX_DEVICE_STORAGE_ACCOUNT = 'OMNI_XBOX_DEVICE'
  ...
  const system = new MicrosoftAccountSystem(logger,
    ...
    new MicrosoftOAuthClient(
      ...
      async (directRedirectToLauncher) => {
        ...
        return (directRedirectToLauncher ? `http://localhost:${port}/auth` : `https://github.com/OmniNodeCo/OmniLauncher-MC/auth?port=${port}`)
      },
      (response) => {
