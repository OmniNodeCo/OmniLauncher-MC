...
export class PeerService extends StatefulService<PeerState> implements IPeerService {
  ...
  private async getMultiplayerInit(): Promise<MultiplayerInitPayload> {
    ...
    const signalingBaseUrl = resolveOmniLauncherApiEndpoints(flights.omnilauncherApiBaseUrl, () =>
      this.app
        .getLogger('MultiplayerApi')
        .warn('Ignoring invalid xmclApiUrl flight; using default OmniLauncher-MC API origins.'),
    ).signaling
    return {
