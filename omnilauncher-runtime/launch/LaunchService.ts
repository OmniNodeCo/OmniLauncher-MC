...
import { access, stat, writeFile } from 'fs-extra'
import { EOL } from 'os'
...
export class LaunchService extends AbstractService implements ILaunchService {
  ...
  async #generateOptions(options: LaunchOptions, version: ResolvedVersion, accessToken?: string) {
    ...
    const launcherName = `OmniLauncher-MC (${this.app.version})`
  ...
  async launch(options: LaunchOptions) {
    // E2E hook: when OMNI_E2E_NO_LAUNCH is set, never spawn a real Java
    ...
    if (process.env.OMNI_E2E_NO_LAUNCH) {
      const fakePid = Math.floor(Math.random() * 100_000) + 100_000
      const operationId = options.operationId || randomUUID()
      const launchId = randomUUID()
      const startTime = Date.now()
      this.log(`[E2E] OMNI_E2E_NO_LAUNCH set; skipping real Java spawn (pid=${fakePid}).`)
      void this.launchHistory.begin(options.gameDirectory, {
        launchId,
        operationId,
        pid: fakePid,
    ...
    try {
      ...
      const startTime = Date.now()
      let launchOutput = ''
      const appendLaunchOutput = (stream: 'stdout' | 'stderr', content: string) => {
        launchOutput += `[${stream}] ${content}`
        if (launchOutput.length > MAX_LAUNCH_OUTPUT_LENGTH) {
