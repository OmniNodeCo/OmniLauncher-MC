...
export class LauncherApp extends EventEmitter {
  ...
  readonly server: Server = createServer((req, res) => {
    this.protocol.handle({
      method: req.method,
      url: new URL(req.url ?? '/', 'omni://launcher'),
      headers: req.headers,
      body: req,
    }).then((resp) => {
  ...
  protected async setup() {
    ...
    if (!this.host.isDefaultProtocolClient('omnilauncher')) {
      const result = this.host.setAsDefaultProtocolClient('omnilauncher')
      if (result) {
  ...
  protected async getStartupUrl(): Promise<string | undefined> {
    if (!IS_DEV && process.platform === 'win32') {
      ...
      if (process.argv.length > 1) {
        ...
        const protocolOption = process.argv.find(a => a.startsWith('omni://'))
        if (protocolOption) {
        ...
        this.logger.log('Didn\'t find omni:// protocol')
      }
    }
