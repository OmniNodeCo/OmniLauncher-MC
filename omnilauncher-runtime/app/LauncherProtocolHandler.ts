 ...
 *
 * The drop in request or localhost request will be transformed into omni:// protocol
 */
  ...
  async handle(request: RequestOptions): Promise<ResolvedResponse> {
    ...
    const context: Context = {
      request: {
        ...
        url: typeof request.url === 'string' ? new URL(request.url, 'omni://launcher') : request.url,
