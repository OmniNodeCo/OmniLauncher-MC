...
describe('formatHelp / formatCommandHelp', () => {
  test('top-level help mentions command', () => {
    const txt = formatHelp(makeRegistry(), { programName: 'omnilauncher-mc' })
    expect(txt).toContain('omnilauncher-mc')
  ...
  test('command help shows positionals + flags', () => {
    ...
    const txt = formatCommandHelp(cmd, { programName: 'omnilauncher-mc' })
