...
> the human maintainers of OmniLauncher-MC and any agent (GitHub Copilot, Claude Code,
...
`omnilauncher-electron-app/main/defaultApp.ts`: `800x400` is the minimum supported
...
2. Treat `pnpm --prefix=omnilauncher-electron-app compile` as a production-bundle
  smoke step, not the default development launch. It writes a
  `HAS_DEV_SERVER=false` bundle to the same
  `omnilauncher-electron-app/dist/index.js`, so manually launching Electron after
...
3. Connect to the launcher's Chrome DevTools endpoint. `omnilauncher-electron-app/dev.ts`
...
5. Renderer changes normally hot-reload. Electron main/runtime changes are
   rebuilt into `omnilauncher-electron-app/dist/index.js` but are not loaded by an
...
pnpm build:renderer                # required after any omnilauncher-ui change
pnpm --prefix=omnilauncher-electron-app compile
