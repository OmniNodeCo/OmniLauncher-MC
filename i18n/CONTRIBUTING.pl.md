...
- xmcl
  - Połączone repozytorium git [launcher-core](https://github.com/OmniNodeCo/minecraft-launcher-core-node) jest submodułem git w tym projekcie.
...
- omnilauncher-electron-app
  ...
  - Bezpośrednio zależy od omnilauncher-runtime.
  - Pośrednio zależy od omnilauncher-ui (tymczasowo, może zostać usunięte później?)
- omnilauncher-ui
...
- omnilauncher-runtime-api
  - To jest współdzielony kod i API dla środowiska wykonawczego OmniLauncher-MC. Może być używany dla aplikacji renderującej (strona przeglądarki)
...
Jeśli jesteś zainteresowany konkretną logiką strony, możesz przejść do `omnilauncher-ui/src/windows/main/views`. Pliki `.vue` w tym folderze są głównymi komponentami używanymi w launcherze. Prefiks pliku to domena interfejsu użytkownika.
...
Jeśli jesteś zainteresowany podstawową logiką, możesz przejść do `omnilauncher-runtime/services/`. Każdy plik w tym miejscu reprezentuje usługę dla konkretnej domeny/aspektu logiki launchera. W trakcie tego procesu powinieneś również zwrócić uwagę na odpowiadające im pliki w `omnilauncher-runtime-api/services/`, które deklarują interfejs rzeczywistych usług.

Kilka przykładów:

1. `omnilauncher-runtime/services/InstanceService.ts` zawiera implementację API do dodawania/usuwania/aktualizacji instancji. `omnilauncher-runtime-api/services/InstanceService.ts` zawiera interfejs `InstanceService`
2. `omnilauncher-runtime/services/InstanceVersionService.ts` zawiera implementację API do sprawdzania stanu wersji instancji. Określi, której wersji będzie używać instancja i czy powinniśmy zainstalować tę wersję.
3. `omnilauncher-runtime/services/InstallService.ts` zawiera implementację API do instalacji Minecraft/Forge/Fabric itp.
4. `omnilauncher-runtime/services/LaunchService.ts` zawiera implementację API do uruchamiania instancji.
...
Powinieneś ustawić `CURSEFORGE_API_KEY` tworząc plik `.env` w `omnilauncher-electron-app`. Ten plik `.env` jest dodany do pliku `.gitignore`.
...
Rdzeń launchera znajduje się w [oddzielnym projekcie](https://github.com/OmniNodeCo/minecraft-launcher-core-node) napisanym w TypeScript.
...
Chyba że kod w `omnilauncher-ui` uległ zmianie, nie musisz budować tego ponownie.
