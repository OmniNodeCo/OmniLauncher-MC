...
- xmcl
  - 链接指向的 git 仓库 [launcher-core](https://github.com/OmniNodeCo/minecraft-launcher-core-node) 是本项目的一个子模块。
  - 实现核心 Minecraft 安装和启动逻辑，并将它们作为库暴露出来。
- omnilauncher-electron-app
  - 用 Electron 实现运行时。
  - 它依赖 omnilauncher-runtime。
  - 它的部分内容依赖 omnilauncher-ui（临时的，可能将在未来被删除）
- omnilauncher-ui
  - 启动器的默认 UI。
...
- omnilauncher-runtime-api
  - 这是 OmniLauncher-MC 运行时的共享代码与 API。它可用于渲染器应用程序（浏览器侧）
...
如果你对核心逻辑感兴趣，你可以看看 `omnilauncher-runtime/services/`。它下面的每个文件都将一个特定领域/业务的逻辑封装成了“服务”。在阅读服务的过程中，你也需要看看 `omnilauncher-runtime-api/services/` 下的对应的服务声明。

一些例子：

1. `omnilauncher-runtime/services/InstanceService.ts`包含添加/删除/更新实例的 API 实现。`omnilauncher-runtime-api/services/InstanceService.ts`包含了`InstanceService`的接口。
2. `omnilauncher-runtime/services/InstanceVersionService.ts`包含检查实例版本健康状况的 API 实现。它将确定实例将使用什么版本，以及我们是否应该安装该版本。
3. `omnilauncher-runtime/services/InstallService.ts`包含安装 Minecraft/Forge/Fabric 等的API实现。
4. `omnilauncher-runtime/services/LaunchService.ts`包含启动实例的 API 实现。
...
#### 设置环境变量
在`omnilauncher-electron-app`下创建`.env`文件来设置`CURSEFORGE_API_KEY`。`.env`文件已被添加到`.gitignore`文件中。
...
启动器核心位于用 TypeScript 编写的[单独项目](https://github.com/OmniNodeCo/minecraft-launcher-core-node)中。
...
除非你在 `omnilauncher-ui` 有新的改动，你已经不需再重新跑这个命令了。
