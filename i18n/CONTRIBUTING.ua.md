...
- xmcl
  - Пов'язане git-репозиторій [launcher-core](https://github.com/OmniNodeCo/minecraft-launcher-core-node) є підмодулем git у цьому проекті.
...
- omnilauncher-electron-app
  ...
  - Це безпосередньо залежить від omnilauncher-runtime.
  - Неявно залежить від omnilauncher-ui (тимчасово, може бути вилучено пізніше?)
- omnilauncher-ui
...
- omnilauncher-runtime-api
  - Це спільний код та API для виконання OmniLauncher-MC. Він може бути використаний для програми рендерингу (на стороні браузера)
...
Якщо вас цікавить конкретна логіка сторінок, ви можете перейти до `omnilauncher-ui/src/windows/main/views`. Файли `.vue` у цій теці є основним компонентом, що використовується у програмі запуску. Префікс файлу - це домен інтерфейсу користувача.
...
Якщо вас цікавить логіка роботи ядра, ви можете перейти до каталогу `omnilauncher-runtime/services/`. Кожен файл у цьому каталозі представляє службу для певного домену/аспекту логіки запуску. Під час цього процесу вам також слід знати про відповідні файли у каталозі `omnilauncher-runtime-api/services/`, які описують інтерфейс відповідних сервісів.

Наведемо кілька прикладів:

1. Файл `omnilauncher-runtime/services/InstanceService.ts` містить реалізацію API для додавання/видалення/оновлення екземплярів. Файл `omnilauncher-runtime-api/services/InstanceService.ts` містить інтерфейс `InstanceService`.
2. Файл `omnilauncher-runtime-api/services/InstanceVersionService.ts` містить реалізацію API для перевірки стану версії екземпляра. Вона визначає, яку версію буде використовувати екземпляр, і чи потрібно встановлювати цю версію.
3. Файл `omnilauncher-runtime/services/InstallService.ts` містить реалізацію API для встановлення Minecraft/Forge/Fabric тощо.
3. `omnilauncher-runtime/services/LaunchService.ts` містить реалізацію API для запуску екземпляра.
...
Вам слід встановити `CURSEFORGE_API_KEY`, створивши файл `.env` у каталозі `omnilauncher-electron-app`. Цей файл `.env` буде додано до файлу `.gitignore`.
...
Ядро панелі запуску знаходиться у [окремому проекті](https://github.com/OmniNodeCo/minecraft-launcher-core-node), написаному на TypeScript.
...
Якщо код під `omnilauncher-ui` не змінився, вам не потрібно збирати його знову.
