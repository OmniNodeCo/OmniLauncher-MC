...
- xmcl
  - Связанный git-репо [launcher-core](https://github.com/OmniNodeCo/minecraft-launcher-core-node) является git-подмодулем в этом проекте.
...
- omnilauncher-electron-app
  ...
  - Это напрямую зависит от omnilauncher-runtime.
  - Неявно зависит от omnilauncher-ui (временно, может быть удалено позже?)
- omnilauncher-ui
...
- omnilauncher-runtime-api
  - Это общий код и API для OmniLauncher-MC runtime. Его можно использовать для приложения рендеринга (на стороне браузера).
...
Если вас интересует логика конкретной страницы, вы можете перейти в папку `omnilauncher-ui/src/windows/main/views`. Файлы `.vue` в этой папке являются основным компонентом, используемым в программе запуска. Префикс файла - это домен пользовательского интерфейса.
...
Если вас интересует логика ядра, вы можете перейти в раздел `omnilauncher-runtime/services/`. Каждый файл в нем представляет собой сервис для определенного домена/аспекта логики запуска. Во время этого процесса вы также должны знать о соответствующих файлах в `omnilauncher-runtime-api/services/`, которые декларируют интерфейс реальных сервисов.

Некоторые примеры:

1. `omnilauncher-runtime/services/InstanceService.ts` содержит реализацию API для добавления/удаления/обновления экземпляров. В файле `omnilauncher-runtime-api/services/InstanceService.ts` содержится интерфейс `InstanceService`.
2. `omnilauncher-runtime/services/InstanceVersionService.ts` содержит реализацию API для проверки работоспособности версии экземпляра. Он определяет, какую версию использует экземпляр, и нужно ли устанавливать эту версию.
3. `omnilauncher-runtime/services/InstallService.ts` содержит реализацию API для установки Minecraft/Forge/Fabric и т.д.
3. `omnilauncher-runtime/services/LaunchService.ts` содержит API-реализацию запуска экземпляра.
...
Вы должны установить `CURSEFORGE_API_KEY`, создав файл `.env` под `omnilauncher-electron-app`. Этот файл `.env` добавляется в файл `.gitignore`.
...
Ядро пусковой установки находится в [отдельном проекте](https://github.com/OmniNodeCo/minecraft-launcher-core-node), написанном на TypeScript.
...
Если только код под `omnilauncher-ui` не изменился, вам не нужно собирать его снова.
