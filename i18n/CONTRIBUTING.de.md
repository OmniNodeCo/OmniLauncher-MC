...
- xmcl
  - Das verknüpfte Git-Repo [launcher-core](https://github.com/OmniNodeCo/minecraft-launcher-core-node) ist ein Git-Submodul des Projekts.
  - Implementiert die Kern-Minecraft-Installations- und Startlogik und stellt sie als Bibliothek zur Verfügung.
- omnilauncher-electron-app
  - Verwendet Electron zur Implementierung der Laufzeitumgebung.
  - Hängt direkt von omnilauncher-runtime ab.
  - Hängt implizit von omnilauncher-ui ab (vorübergehend, wird vielleicht später entfernt?)
- omnilauncher-ui
  - Das Haupt-Standard-UI des Launchers.
  - 100% browserkompatibel. Keine Electron-API involviert.
- omnilauncher-runtime
  - Die Kernimplementierung der Launcher-Architektur. Hängt nur von Node.js ab und erfordert keine Electron-Laufzeitumgebung.
- omnilauncher-runtime-api
  - Der gemeinsam genutzte Code & die API der OmniLauncher-MC-Laufzeitumgebung. Er kann für Renderer-Apps (Browserseitig) verwendet werden.
...
Wenn du an einer spezifischen Seitenlogik interessiert bist, sehe dir gerne `omnilauncher-ui/src/windows/main/views` an. Die `.vue`-Dateien in diesem Ordner sind die Hauptkomponenten, die der Launcher verwendet. Das Präfix der Datei entspricht der Domäne der UI.
...
Wenn du an der Kernlogik interessiert sind, kannst du dir `omnilauncher-runtime/services/` ansehen. Jede Datei dort repräsentiert einen Dienst für einen spezifischen Aspekt der Launcher-Logik. Währenddessen solltest du auch auf die entsprechenden Dateien unter `omnilauncher-runtime-api/services/` achten, die die Schnittstellen der tatsächlichen Dienste definieren.

Einige Beispiele:

1. `omnilauncher-runtime/services/InstanceService.ts` enthält die API-Implementierung zum Hinzufügen/Entfernen/Aktualisieren von Instanzen. `omnilauncher-runtime-api/services/InstanceService.ts` enthält die Schnittstelle des `InstanceService`.
2. `omnilauncher-runtime/services/InstanceVersionService.ts` enthält die API-Implementierung zur Überprüfung des Gesundheitszustands der Instanzversion. Sie bestimmt, welche Version die Instanz verwenden wird und ob diese installiert werden sollte.
3. `omnilauncher-runtime/services/InstallService.ts` enthält die API-Implementierung zur Installation von Minecraft/Forge/Fabric usw.
3. `omnilauncher-runtime/services/LaunchService.ts` enthält die API-Implementierung zum Starten einer Instanz.
...
```bash
git clone --recurse-submodules https://github.com/OmniNodeCo/OmniLauncher-MC
```
...
Du solltest `CURSEFORGE_API_KEY` festlegen, indem du eine `.env`-Datei unter `omnilauncher-electron-app` erstellst. Diese `.env`-Datei wird der `.gitignore`-Datei hinzugefügt.
...
Der Launcher-Kern befindet sich in einem [separaten Projekt](https://github.com/OmniNodeCo/minecraft-launcher-core-node), das in TypeScript geschrieben wurde.
...
Sofern sich der Code unter `omnilauncher-ui` nicht geändert hat, musst du diesen nicht erneut kompilieren.
